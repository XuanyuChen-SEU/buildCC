...
class WorktreeManager:
    ...
    def _run_git(self, args: list[str]) -> str:
        if not self.git_available:
            raise RuntimeError(f"Error: Git is not available in {self.repo_root}")
        r = subprocess.run(
            ["git", *args],
            cwd=self.repo_root,
            capture_output=True,
    ...
    def create(self, name: str, task_id: int = None, base_ref: str = "HEAD") -> str:
        ...
        try:
            self._run_git(["worktree", "add", "-B", branch, path, base_ref])
    ...
    def status(self, name: str) -> str:
        ...
        r = subprocess.run(
            ["git", "status", "--short", "--porcelain", path],
            cwd=self.repo_root,
            capture_output=True,
            text=True,
            timeout=60,
    ...
    def run(self, name: str, command: str) -> str:
        ...
        if any(d in command.lower() for d in dangerous):
            return f"Error: Dangerous command: {command}"
        ...
        try:
            ...
            return f"Error: Command timed out(300s): {command}"
        except Exception as e:
            return f"Error: Command failed: {command}\n{str(e)}"
    ...
    def remove(
        ...
        try:
            ...
            self.events.emit(
                ...
                task={"id": wt["task_id"]} if wt["task_id"] is not None else {},
                worktree={"name": name, "path": wt["path"], "status": "removed"},
            )
            ...
            self.events.emit(
                ...
                task={"id": wt["task_id"]} if wt["task_id"] is not None else {},
                worktree={"name": name, "path": wt["path"]},
                error=str(e),
            )
            raise
...
TOOLS = [
    ...
    {
        "name": "task_update",
        "description": "Update the status or owner of a task.",
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        "description": "Create a git worktree and optionallt bind it to a task.",
        "input_schema": {
            "type": "object",
            "properties": {
                "name": {"type": "string"},
    ...
    },
    {
        ...
        "description": "Get the git status of a specific worktree by name.",
        "input_schema": {
            "type": "object",
    ...
    },
    {
        ...
        "description": "Remove a worktree and optinally mark its bound task completed.",
        "input_schema": {
            "type": "object",
            "properties": {
                "name": {"type": "string"},
...
if __name__ == "__main__":
    ...
    if not WORKTREES.git_available:
        print("Error: Not a git repository, worktree_* tools will return errors")
        exit(1)
    history = []
    while True:
        try:

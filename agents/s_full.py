...
毕业级实现：融合 s01-s11 的全部机制。
s12（任务感知的 worktree 隔离）单独讲解。
这不是教学分步课，而是“把所有东西拼在一起”的参考实现。
    ...
    |  +--------+----------+----------+---------+-----------+          |
    |                                                                   |
    |  子智能体（s04）：spawn -> 工作 -> 返回摘要                         |
    |  队友（s09）：spawn -> 工作 -> idle -> 自动认领（s11）             |
    |  关闭流程（s10）：request_id 握手                                  |
    |  计划闸门（s10）：提交 -> 批准/拒绝                                |
    +------------------------------------------------------------------+
